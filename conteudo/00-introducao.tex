...
O desenvolvimento desse instrumental para contabilizar as pegadas ambientais permitiria, além da construção
de novas intervenções sobre cargas de trabalho, a medição do teto factível de redução de impacto ambiental.
O teto teórico máximo é tem seu cálculo possibilitado através da otimização máxima possível para cada
job de uma carga de trabalho, ou seja, para cada job de duração estimada T, escolhe-se a janela de tamanho
T com menor intensidade em todo o traço. Porém, esse cálculo não é realista para o mundo real, visto que 
além de ignorar a capacidade de um sistema de HPC, que tem um número fixo de nós durante o tempo, além de 
considerar que um job poderia ser alocado para uma janela anterior a sua própria submissão. O teto factível
é obtido então a partir do conhecimento do traço inteiro de maneira clarividente em conjunto com escolhas
gulosas pelas melhores janelas respeitando capacidade, chegadas e reservas. O simulador garante que o resultado
é atingível no mundo dado que ele efetivamente construiu um escalonamento que o atinge.

É justamente a partir desses tetos que é possível obter a dimensão real da qualidade de uma heurística de
escalonamento. Sem isso, reportar 3\% de reduções nas emissões de carbono pode parecer um ganho irrisório.
Porém, por exemplo, caso seja estabelecido que o teto de redução nas emissões de carbono é de 4\%, a heurística
na verdade acaba se mostrando muito competente, visto que alcançou 75\% das reduções disponíveis.

TESE.

\section{Objetivo e questões de pesquisa}

Este trabalho tem como objetivo delimitar o espaço de redução máximo nas emissões de carbono
e consumo de água acessível a um escalonador em lote de sistemas HPC, e também determinar
qual fração desse espaço é alcançável através da implementação de políticas simples de escalonamento,
sem o uso de modelos preditivos treinados. Para isso, foi desenvolvido um modelo de contabilização
de pegadas ambientais integrado a um simulador de escalonamento de sistemas HPC, construído um 
ambiente experimental a partir de sinais de redes reais e traços de cargas de trabalho de sistemas 
HPC em produção, e por fim, avaliadas políticas que diferem entre si quanto ao mecanismo de decisão
e quanto ao grau de informação sobre o comportamento futuro desses sinais ambientais.

Consequentemente, a questão de pesquisa primária que este trabalho visa responder é \emph{"Qual é 
o espaço de redução nas emissões de carbono e consumo de água acessível a um escalonador em lote
de sistemas HPC, e que fração dele heurísticas simples conseguem capturar?"}. A partir dessa questão
primária, é possível derivar outras cinco questões subordinadas, com as duas primeiras estabalecendo
o referencial teórico e factível discorrido anteriormente, e as três seguintes avaliando políticas
de escalonamento contra esse referencial.

\begin{description}
    \item[QP1] Como a composição da matriz elétrica de uma região determina o teto teórico, ao longo do dia,
mês e das estações?
    \item[QP2] Quanto do teto teórico total permanece factível quando consideradas as restrições
práticas de escalonamento?
    \item[QP3] Os objetivos de redução das emissões de carbono e redução do consumo de água são 
convergentes ou divergentes?
    \item[QP4] Quanta informação sobre intensidades futuras é necessária para capturar o teto factível
    \item[QP5] -   
\end{description}

Define-se como \emph{simples} uma política de escalonamento cuja regra de decisão não envolve a utilização
de um ou mais modelos preditivos ajustados aos dados, e também cujo custom computacional por decisão seja aritmético.
O grau de informação sobre intensidades futuras não se encaixa nessa definição, dado que ele é tratado
como um dos fatores experimentais, variado em níveis declarados de maneira explícita. Defini-se como 
\emph{competitiva} uma política que promove a redução da pegada operacional em relação a pegada gerada
ao seguir a política baseline EASY backfilling sem aumentar desbalanceadamente o mean bounded slowdown.
O escalonamento por si só não é capaz de alterar o tempo de execução de um job, tendo o controle apenas
da quantidade que um job espera na fila de execução.

Este trabalho não considera a migração geográfica dos jobs entre diferentes sistemas de HPC, não modela
a variação de potência intra-job e também não trata o gerenciamento de potência em execução de hardware.

\section{Contribuições}

\begin{description}
    \item[Modelo de contabilização de pegada] Extensões nos simuladores de escalonadores de HPC para
contabilizar por job as pegadas de carbono e hídrica, tanto operacionais quando incorporadas, instante
a instante ao logo da execução do mesmo. Torna possível comparar escalonamentos alternativos quanto
à pegada, e não apenas quanto à energia exclusivamente. Essa distinção é necessária visto que a intensidade
de carbono/água varia dentro da própria execução do job. 
    \item[Ambiente experimental reprodutível] Sinais de intensidade de emissões de carbono e consumo de água
derivados de geração real de três países com composições elétricas diferentes, em resolução de 15 minutos,
combinados a traços de carga de produção de sistemas HPC e plataformas modeladas para uso dos simuladores.
Cada execução pode ser repetida e reparametrizada, além da possibilidade de substituição do modelo de potência,
sinal de rede ou carga sem reconstruir o pipeline.
    \item[Referencial de teto] O teto teórico, calculado analiticamente baseado em um cenário não alcançável na prática,
e o teto factível, obtido através de um escalonador clarividente. Torna uma redução relatada interpretável,
observado que sem um referencial, não é possível realizar a distinção entre uma política que aproveitou
boa parte do espaço disponível em relação a uma política que aproveitou pouco de um espaço com amplas margens.
    \item[Mapeamento sistemático dos impactos ambientais em HPC] Um mapeamento sistemático dos impactos ambientais
em computação de alto desempenho, visando entender o que pesquisadores tem buscado entender e como intervir.
O autor desta dissertação participou da triagem manual dos registros candidatos.
\end{description}